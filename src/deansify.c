/* ABC: 5/9/91: I did not write this program (de-ansify).
 * I found it lying around, unfortunately, I do not know who originally
 * wrote it, so am leaving it unattributed....
 * (This IS public domain code --- most probably from dvips or xgl.)
 * [abc] modified to handle the %{ starting brace of ilex.l and iyacc.y
 ***************************************************************************
 */
...
int c;
main() {
int possible=0, braces=0, gotslash=0, gotpercent=0, gotstar=0, gotnl=0;
    for (c=getchar(); c != -1;) {
	if (gotslash && (c=='*')) {
	    putchar(c);
	    c=getchar();
